...
def input_data(f_name):
     global column_names
     s_time_frame = '2019-01-01 2020-12-31'
...
def top_10_trending():
    ...
    df.to_excel(writer, index = False, startcol = 0, startrow = 2)
    worksheet = writer.sheets['Sheet1']
    workbook = writer.book
    cell_format = workbook.add_format({'size':15, 'align':'center'})
    worksheet.merge_range('A1:F1',"Danh sách từ khóa tìm kiếm tại Việt Nam",cell_format)
    worksheet.merge_range('A2:F2',"Năm 2020",cell_format)
...
def build_sql(year=2020, limit=10):
    sql = """SELECT A.keyword, A.sum_val, B.monthly, B.max_val
            FROM
...
def top_10_trending_2020():
     ...
     df = pd.DataFrame(rd, columns=['keyword','sum_val', 'monthly', 'max_val'])
     df2 = pd.DataFrame(rd2, columns=['keyword','sum_val', 'monthly','max_val'])

     df_new = df.rename(columns={'keyword': 'Keyword', 'sum_val': 'Số lần tìm kiếm', 'monthly':'Tháng tìm kiếm nhiều nhất','max_val': 'Số lần trong tháng'})
     df_new2 = df2.rename(columns={'keyword': 'Keyword', 'sum_val': 'Số lần tìm kiếm', 'monthly':'Tháng tìm kiếm nhiều nhất','max_val': 'Số lần trong tháng'})
    
     piv = df_new.pivot_table(index=["Keyword", "Tháng tìm kiếm nhiều nhất"], values=["Số lần tìm kiếm", "Số lần trong tháng"], aggfunc='sum')
     piv2 = df_new2.pivot_table(index=["Keyword", "Tháng tìm kiếm nhiều nhất"], values=["Số lần tìm kiếm", "Số lần trong tháng"], aggfunc='sum')
     ...
     df_new2.to_excel(writer,sheet_name='Sheet1', startcol = 5, startrow = 2, index = False)
     ...
     worksheet.merge_range('A1:I1',"Danh sách từ khóa tìm kiếm tại Việt Nam",cell_format)
     worksheet.merge_range('A2:D2',"Năm 2020",cell_format)
     worksheet.merge_range('E2:I2',"Năm 2019",cell_format)
